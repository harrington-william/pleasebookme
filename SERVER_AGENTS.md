...
Established across `UserEntity`, `RoleEntity`, `PermissionEntity`, `AccountEntity`, `ApiKeyEntity`, `RefreshTokenEntity`, `TenantEntity`, `EcosystemEntity`, `TenantPlanEntity`, `TenantDomainEntity`:
...
- **Multiple unresolved FKs on one table, resolved incrementally**: `TenantEntity` (`tenant.tenants`, `V24__tenant_tenants.sql`) FKs to four different tables. `organization_id` → `organization.organizations` still stays a plain `BigInteger` field since `OrganizationEntity` doesn't exist yet. `owner_user_id` → `auth.users` was always a real `@ManyToOne UserEntity` since `UserEntity` already existed. `ecosystem_id` → `tenant.ecosystems` and `plan_id` → `tenant.plans` both started as plain `BigInteger`, then were each upgraded — to `@ManyToOne EcosystemEntity ecosystem` and `@ManyToOne TenantPlanEntity plan` respectively — once their target entity got implemented. When a sibling entity for an existing raw-FK field is implemented, go back and upgrade that field to the real relationship rather than leaving it as a `BigInteger`. Apply the "does the target entity exist yet" test per-column, not per-table.
